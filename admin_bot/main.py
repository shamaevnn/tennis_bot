...
from admin_bot.payment.manage_data import (
    PAYMENT_YEAR,
    PAYMENT_YEAR_MONTH,
...
payment_handler = ConversationHandler(
    entry_points=[
        CallbackQueryHandler(
            payment_handlers.change_payment_data, pattern=f"{PAYMENT_START_CHANGE}"
        )
    ],
    states={
        payment_handlers.START_CHANGE_PAYMENT: [
            MessageHandler(Filters.text, payment_handlers.get_id_amount)
        ],
        payment_handlers.CONFIRM_OR_CANCEL: [
            CallbackQueryHandler(
                payment_handlers.confirm_or_cancel_changing_payment,
                pattern=f"^{PAYMENT_CONFIRM_OR_CANCEL}",
            )
        ],
    },
    fallbacks=[
        ...
        CallbackQueryHandler(
            payment_handlers.month_payment, pattern="^{}".format(PAYMENT_YEAR_MONTH)
        ),
    ],
)
send_message_handler = ConversationHandler(
    entry_points=[
        CallbackQueryHandler(
            send_message_handlers.select_groups_where_should_send,
            pattern=f"^{SEND_MESSAGE}",
        ),
        MessageHandler(
            Filters.regex(f"{ADMIN_SEND_MESSAGE}"),
            send_message_handlers.select_groups_where_should_send,
        ),
    ],
    states={
        send_message_handlers.GROUP_IDS: [
            CallbackQueryHandler(
                send_message_handlers.text_to_send, pattern=f"^{SEND_MESSAGE}"
            )
        ],
        send_message_handlers.TEXT_TO_SEND: [
            MessageHandler(Filters.text, send_message_handlers.receive_text_and_send)
        ],
    },
    fallbacks=[
        CommandHandler("cancel", cancel),
...
def setup_dispatcher(dp):
    ...
    dp.add_handler(
        MessageHandler(
            Filters.regex(f"{ADMIN_PAYMENT}"), payment_handlers.start_payment
        )
    )
    dp.add_handler(
        CallbackQueryHandler(
            payment_handlers.start_payment, pattern=f"^{ADMIN_PAYMENT}"
        )
    )
    ...
    dp.add_handler(
        CallbackQueryHandler(
            permission_for_ind_train, pattern="^{}".format(PERMISSION_FOR_IND_TRAIN)
        )
    )
    dp.add_handler(
        CallbackQueryHandler(
            save_many_ind_trains, pattern="^{}".format(AMOUNT_OF_IND_TRAIN)
        )
    )
    ...
    dp.add_handler(
        CallbackQueryHandler(
            permission_for_rent_court, pattern=f"^{PERMISSION_FOR_RENT_COURT}"
        )
    )
    ...
    dp.add_handler(
        CallbackQueryHandler(
            show_trainingroupday_info, pattern="^{}".format(SHOW_GROUPDAY_INFO)
        )
    )
    dp.add_handler(
        CallbackQueryHandler(
            payment_handlers.year_payment, pattern="^{}".format(PAYMENT_YEAR)
        )
    )
    dp.add_handler(
        CallbackQueryHandler(
...
dispatcher_admin = setup_dispatcher(
    Dispatcher(admin_bot, None, workers=0, use_context=True)
)
