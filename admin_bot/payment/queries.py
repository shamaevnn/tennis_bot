...
def get_total_paid_amount_for_month(year: int, month: int) -> int:
    amount_for_this_month = Payment.objects.filter(year=year, month=month).aggregate(
        sigma=Sum("fact_amount")
    )
...
def get_total_should_pay_amount(year: Union[str, int], month: Union[str, int]) -> int:
    should_pay_this_month = (
        TrainingGroup.objects.annotate(
            count_tr_days=Count(
                "grouptrainingday",
                filter=Q(
            ...
            count_players=Count("players", distinct=True),
        )
        .filter(max_players__gt=1)
        .annotate(
...
def get_not_paid_payments(
    year: Union[str, int], month: Union[str, int]
) -> QuerySet[Payment]:
    payments = (
        Payment.objects.filter(
            player__status=Player.STATUS_TRAINING, fact_amount=0, month=month, year=year
        )
        .annotate(
        ...
        .filter(group_status=TrainingGroup.STATUS_GROUP)
    )
    return payments
