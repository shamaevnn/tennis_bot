...
from .static_text import WILL_SAY_RENT_COURT_IS_CANCELLED, RENT_COURT_IS_ALREADY_CANCELLED, WILL_SAY_RENT_IS_ACCEPTED
...
def permission_for_rent_court(update: Update, context):
    permission, user_id, tr_day_id = update.callback_query.data[len(PERMISSION_FOR_RENT_COURT):].split('|')
    ...
    if tr_day.exists():
        ...
        if permission == PERMISSION_YES:
            user_text = COACH_ACCEPTED_RENT_COURT.format(date_info=date_info)
            ...
            user_text = COACH_CANCELLED_RENT_COURT.format(attention=ATTENTION, date_info=date_info)
            tr_day.delete()
