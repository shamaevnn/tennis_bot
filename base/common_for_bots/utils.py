...
def bot_edit_message(
    bot: Bot, text: str, update: Update, markup: Optional[InlineKeyboardMarkup] = None
):
...
def create_calendar(
    ...
    data_ignore = create_callback_data(
        purpose_of_calendar, CLNDR_IGNORE, year, month, 0
    )
    keyboard = []
    # First row - Month and Year
    row = [
        InlineKeyboardButton(
            from_digit_to_month[month] + " " + str(year), callback_data=data_ignore
        )
    ]
    keyboard.append(row)
    ...
    for week in my_calendar:
        ...
        for day in week:
            if day == 0:
                ...
                day_info = str(day)
                if dates_to_highlight and (
                    date(year, month, day) in dates_to_highlight
                ):
                    day_info = f"{str(day)}✅"
                row.append(
                    InlineKeyboardButton(
