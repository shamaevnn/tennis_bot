...
from .django_admin.utils import send_alert_about_changing_tr_day_time


from .utils.change_available_status import change_tr_day_available_status_and_send_alert
    ...
    def clean(self):
        if "status" in self.changed_data:
            ...
            if current_status == Player.STATUS_WAITING and (
                new_status == Player.STATUS_ARBITRARY
                or new_status == Player.STATUS_TRAINING
            ):
...
class TrainingGroupForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if players.count() > max_players:
            raise ValidationError(
                {
                    "max_players": ERROR_LIMIT_MAX_PLAYERS.format(
            ...
            tr_day = (
                ...
                .annotate(
                    start=ExpressionWrapper(
                        F("start_time") + F("date"), output_field=DateTimeField()
                    ),
    ...
    def clean(self):
        def send_alert_about_cancel_in_visitors(self, type_of_visitors="visitors"):
            ...
            if type_of_visitors == "visitors":
                if (
                    self.cleaned_data.get(type_of_visitors).count()
                    < self.instance.visitors.count()
                ):
                ...
                if (
                    self.cleaned_data.get(type_of_visitors).count()
                    < self.instance.pay_visitors.count()
                ):
                ...
                if (
                    self.cleaned_data.get(type_of_visitors).count()
                    < self.instance.pay_bonus_visitors.count()
                ):
            ...
            if canceled_players:
                text = CANCEL_TRAIN_PLUS_BONUS_LESSON.format(
        ...
        current_amount_of_players = (
            self.cleaned_data.get("visitors").count()
            + self.cleaned_data.get("pay_visitors").count()
            + self.cleaned_data.get("pay_bonus_visitors").count()
            + group.players.count()
            - self.cleaned_data.get("absent").count()
        )

        if group.available_for_additional_lessons:
            if current_amount_of_players > 6:
                raise ValidationError(
                    ERROR_LIMIT_MAX_PLAYERS.format(6, current_amount_of_players)
                )
        else:
            if current_amount_of_players > group.max_players:
                raise ValidationError(
                    ERROR_LIMIT_MAX_PLAYERS.format(
                        group.max_players, current_amount_of_players
                    )
                )

        if (
            "start_time" in self.changed_data
            or "duration" in self.changed_data
            or "date" in self.changed_data
        ):
            """
            ...
            на уже существущие.
            """
            exist_trainings = GroupTrainingDay.objects.filter(
                date=self.cleaned_data.get("date")
            )
            start_time = datetime.combine(
                self.cleaned_data.get("date"), self.cleaned_data.get("start_time")
            )

            for train in exist_trainings:
                exist_train_start_time = datetime.combine(train.date, train.start_time)
                if (
                    exist_train_start_time
                    <= start_time
                    < exist_train_start_time + train.duration
                ):
                    raise ValidationError(
                        ERROR_CANT_ADD_NEW_TRAIN.format(
                            train.start_time, train.duration
                        )
                    )

            # send alert to players about changing lesson parameters
            # check only existing tr_days
            if GroupTrainingDay.objects.filter(
                ...
                if "start_time" in self.changed_data:
                    ...
                    before_after_text += self.instance.start_time.strftime(
                        TM_TIME_SCHEDULE_FORMAT
                    )
                ...
                if "date" in self.changed_data:
                    ...
                    before_after_text += (
                        f" 🔜 {self.cleaned_data.get('date').strftime(DT_BOT_FORMAT)}"
                    )

                day_of_week = from_eng_to_rus_day_week[
                    calendar.day_name[self.instance.date.weekday()]
                ]
                text = (
                    f"⚠️ATTENTION⚠️\n"
