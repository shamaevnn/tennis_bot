...
class CreateData:
    ...
    def group_player(cls, tg_id: int, first_name: str, **kwargs) -> Player:
        player = Player.objects.create(tg_id=tg_id, first_name=first_name, status=Player.STATUS_TRAINING, **kwargs)
        return player

    @classmethod
    def arbitrary_player(cls, tg_id: int, first_name: str, **kwargs) -> Player:
        player = Player.objects.create(tg_id=tg_id, first_name=first_name, status=Player.STATUS_ARBITRARY, **kwargs)
        return player
    ...
    @classmethod
    def tr_day_for_group(cls, group, date=None, start_time=None, **kwargs) -> GroupTrainingDay:
        if date is None:
