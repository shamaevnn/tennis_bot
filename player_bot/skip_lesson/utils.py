...
from player_bot.skip_lesson.static_text import PLAYER_CANCELLED_IND_TRAIN, PLAYER_SKIPPED_TRAIN_FOR_BONUS, \
    ...
    CANT_CANCEL_LESSON_TOO_LATE, OKAY_TRAIN_CANCELLED, PLAYER_CANCELLED_RENT_COURT
...
def handle_skipping_train(training_day: GroupTrainingDay, player: Player, date_info):
    ...
    if training_day.tr_day_status == GroupTrainingDay.RENT_COURT_STATUS:
        training_day.delete()
        admin_text = PLAYER_CANCELLED_RENT_COURT.format(ATTENTION, player.first_name, player.last_name, date_info)
        return text, admin_text

    if datetime.combine(training_day.date, training_day.start_time) - moscow_datetime(
            datetime.now()) < player.time_before_cancel:
        text = CANT_CANCEL_LESSON_TOO_LATE.format(round(player.time_before_cancel.seconds / 3600))
        admin_text = ""
        return text, admin_text

    if training_day.is_individual:
        training_day.delete()
        admin_text = PLAYER_CANCELLED_IND_TRAIN.format(ATTENTION, player.first_name, player.last_name, date_info)
    else:
        if player in training_day.visitors.all():
            player.bonus_lesson += 1
            training_day.visitors.remove(player)
            admin_text = PLAYER_SKIPPED_TRAIN_FOR_BONUS.format(player.first_name, player.last_name, date_info)
            ...
            admin_text = PLAYER_SKIPPED_TRAIN_FOR_MONEY.format(player.first_name, player.last_name, date_info)
        elif player in training_day.pay_bonus_visitors.all():
            player.bonus_lesson += 1
            training_day.pay_bonus_visitors.remove(player)
            admin_text = PLAYER_SKIPPED_TRAIN_FOR_PAY_BONUS.format(player.first_name, player.last_name, date_info)
        else:
            player.bonus_lesson += 1
            training_day.absent.add(player)
            admin_text = PLAYER_SKIPPED_TRAIN_IN_HIS_GROUP.format(player.first_name, player.last_name, date_info)
        player.save()
    return text, admin_text
