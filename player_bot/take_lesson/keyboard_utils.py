...
from base.common_for_bots.utils import DT_BOT_FORMAT, TM_TIME_SCHEDULE_FORMAT, create_callback_data
...
def choose_type_of_training_keyboard():
    ...
    buttons = [[
        InlineKeyboardButton(TYPE_IND, callback_data=f"{data}{manage_data.TRAINING_IND}")
    ], [
        InlineKeyboardButton(TYPE_GROUP, callback_data=f"{data}{manage_data.TRAINING_GROUP}")
    ], [
        InlineKeyboardButton(TYPE_RENT, callback_data=f"{data}{manage_data.TRAINING_RENT}")
    ]]
    ...
    buttons.append([
        InlineKeyboardButton(
            ...
            callback_data=create_callback_data(
                f'{back_button_data}{duration}', CLNDR_ACTION_BACK, day_date.year, day_date.month, 0
            )
        )
    ])
    return InlineKeyboardMarkup(buttons)
