...
from tele_interface.static_text import NO_PLACES_FOR_THIS_TIME_CHOOSE_ANOTHER, CHOOSE_TYPE_OF_PAYMENT
from tele_interface.take_lesson.keyboard_utils import back_to_group_times_when_no_left_keyboard, \
    choose_type_of_payment_for_group_lesson_keyboard
from tele_interface.take_lesson.utils import handle_taking_group_lesson
...
class NoFreePlacesTests(TestCase):
    ...
    def setUp(self):
        self.group_user_with_bonus_lessons = create_group_user(id=350490234, first_name='Nikita 1', bonus_lesson=10)
        self.group_user_without_bonus_lessons = create_group_user(id=350490235, first_name='Nikita 2', bonus_lesson=0)
        self.arbitrary_user_with_bonus_lessons = create_arbitrary_user(id=350490236, first_name='Nikita 3', bonus_lesson=10)
        self.arbitrary_user_without_bonus_lessons = create_arbitrary_user(id=350490236, first_name='Nikita 4', bonus_lesson=0)
        ...
        self.group_6_max_players = create_group(max_players=6)
        self.group_4_max_players.users.add(self.user_1, self.user_2, self.user_3, self.user_4)
        self.group_6_max_players.users.add(self.user_1, self.user_2, self.user_3, self.user_4, self.user_5, self.user_6)

        self.group_4_max_players_ad_l = create_group(max_players=4, available_for_additional_lessons=True)
        self.group_6_max_players_ad_l = create_group(max_players=6, available_for_additional_lessons=True)
        self.group_4_max_players_ad_l.users.add(self.user_1, self.user_2, self.user_3, self.user_4)
        self.group_6_max_players_ad_l.users.add(self.user_1, self.user_2, self.user_3, self.user_4, self.user_5, self.user_6)

    def test_no_free_places_no_additional_lessons(self):
        tr_day_4 = create_tr_day_for_group(self.group_4_max_players)
        tr_day_6 = create_tr_day_for_group(self.group_6_max_players)

        user_text, user_markup, admin_text, admin_markup = handle_taking_group_lesson(
            self.group_user_with_bonus_lessons, tr_day_4)
        self.assert_no_places_choose_another(user_text, user_markup, admin_text, admin_markup, tr_day_4)
        user_text, user_markup, admin_text, admin_markup = handle_taking_group_lesson(
            self.group_user_without_bonus_lessons, tr_day_4)
        self.assert_no_places_choose_another(user_text, user_markup, admin_text, admin_markup, tr_day_4)
        user_text, user_markup, admin_text, admin_markup = handle_taking_group_lesson(
            self.arbitrary_user_with_bonus_lessons, tr_day_4)
        self.assert_no_places_choose_another(user_text, user_markup, admin_text, admin_markup, tr_day_4)
        user_text, user_markup, admin_text, admin_markup = handle_taking_group_lesson(
            self.arbitrary_user_without_bonus_lessons, tr_day_4)
        self.assert_no_places_choose_another(user_text, user_markup, admin_text, admin_markup, tr_day_4)

        user_text, user_markup, admin_text, admin_markup = handle_taking_group_lesson(
            self.group_user_with_bonus_lessons, tr_day_6)
        self.assert_no_places_choose_another(user_text, user_markup, admin_text, admin_markup, tr_day_6)
        user_text, user_markup, admin_text, admin_markup = handle_taking_group_lesson(
            self.group_user_without_bonus_lessons, tr_day_6)
        self.assert_no_places_choose_another(user_text, user_markup, admin_text, admin_markup, tr_day_6)
        user_text, user_markup, admin_text, admin_markup = handle_taking_group_lesson(
            self.arbitrary_user_with_bonus_lessons, tr_day_6)
        self.assert_no_places_choose_another(user_text, user_markup, admin_text, admin_markup, tr_day_6)
        user_text, user_markup, admin_text, admin_markup = handle_taking_group_lesson(
            self.arbitrary_user_without_bonus_lessons, tr_day_6)
        self.assert_no_places_choose_another(user_text, user_markup, admin_text, admin_markup, tr_day_6)

    def test_no_free_places_with_all_kinds_of_visitors_and_available_additional_lessons(self):
        tr_day_4 = create_tr_day_for_group(self.group_4_max_players_ad_l)
        ...
        tr_day_6 = create_tr_day_for_group(self.group_6_max_players_ad_l)
        ...
        self.assertIsNone(admin_markup)
        self.assertIn(self.group_user_without_bonus_lessons, tr_day_4.pay_visitors.all())
        ...
        self.assertIsNone(user_markup)
        self.assertIsNone(admin_markup)
        self.assertIn(self.arbitrary_user_without_bonus_lessons, tr_day_4.pay_visitors.all())
